...
const VerifyOtpSignup = () => {
  ...
  useEffect(() => {
  const expiryTime = localStorage.getItem("signupOtpExpiryTime");
  const now = Date.now();
  ...
  if (expiryTime) {
    ...
    if (remaining > 0) {
      ...
      localStorage.setItem("signupOtpExpiryTime", newExpiry);
      setTimeLeft(120);
      setDigits(["", "", "", "", "", ""]);
    }
  } else {
...
//     } else {
//       // No stored timer → set new expiry
//       const newExpiry = Date.now() + 120 * 1000;
//       localStorage.setItem("signupOtpExpiryTime", newExpiry);
//     }
//   }, []);
...
//       localStorage.removeItem("signupOtpExpiryTime");
