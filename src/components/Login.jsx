...
import { toast } from "react-toastify";
import { Link, useNavigate, Navigate } from "react-router-dom";
import { setToken } from "../utills/check token";
...
const Login = () => {
  ...
  // ✅ Check token OUTSIDE of useEffect
  const token =
    typeof window !== "undefined" ? localStorage.getItem("token") : null;
  ...
  useEffect(() => {
    // ✅ Only load Google script (side effect)
    ...
    <div className="relative flex min-h-screen items-center justify-center p-4 bg-gray-900">
      ...
      <div className="relative z-10 flex max-w-4xl w-full mx-auto shadow-2xl rounded-xl overflow-hidden">
        ...
        <div className="flex-1 relative p-10 text-white bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 hidden lg:block">
          <h1 className="text-4xl font-bold mb-4">Welcome to Connectify</h1>
          <p className="text-md font-light">
            Log in to join your community. Connect with peers, share insights, and build relationships that power your success.
          </p>
        </div>
          ...
          >
            {({ isSubmitting, errors, touched }) => (
              <Form>
                ...
                <div className="flex justify-end mb-6 text-sm">
                  <Link to="/forgot" className="text-purple-600 hover:text-purple-800 transition duration-150">
                    Forgot password?
                  </Link>
                </div>
                ...
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full py-3 text-white font-semibold rounded-lg bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 transition duration-150 ease-in-out cursor-pointer"
                >
                  {isSubmitting ? "Logging in..." : "LOGIN"}
                </button>
